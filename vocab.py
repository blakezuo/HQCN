...
class Vocab(object):
    ...
    def __init__(self, filename=None, initial_tokens=None, lower=True):
        ...
        if filename is not None:
            self.load_from_embed_file(filename)
    ...
    def load_from_embed_file(self, file_path):
    ...
    def init_pretrained_embeddings(self, embed_dim, pretrained_embedding):
        """
        initializes the embeddings with pretrained embedding for each token 
        Args:
        ...
        #self.pretrained = {}
        pretrained_count = 0
        with open(pretrained_embedding, 'r') as f:
            for line in f:
                line = line.strip().split()
                if line[0] not in self.token2id:
                    continue
                word_id = self.get_id(line[0])
                for i in range(self.embed_dim):
                    try:
                        self.embeddings[word_id][i] = float(line[i+1].strip())
                    except:
                        print(line)
                pretrained_count += 1
        print("Word with pretrained_embedding: %d/%d" % (pretrained_count, self.size()))
